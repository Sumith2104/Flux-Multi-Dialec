...
export function CreateScraperDialog({ projectId, onCreated }: { projectId: string; onCreated: () => void }) {
    ...
    const handleSubmit = async () => {
        ...
        // Convert mappings array into the expected JSON object
        ...
        try {
            ...
            if (data.success) {
                toast({ title: 'Scraper Created', description: 'New scraper job added successfully.' });
                setOpen(false);
                onCreated();
            } else {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button><Plus className="h-4 w-4 mr-2" /> New Scraper Job</Button>
            </DialogTrigger>
            <DialogContent className="max-w-2xl px-6 py-4 max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>Create Scraper Job</DialogTitle>
                    <DialogDescription>Configure a new web data extraction pipeline.</DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-6 mt-4">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-muted-foreground">Table will be auto-generated if missing.</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3 pt-4 border-t">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-0.5">Map CSS query selectors to database columns.</p>
                            </div>
                        ...
                        {mappings.map((mapping, idx) => (
                            <div key={idx} className="flex gap-2 items-center bg-muted/20 p-2 rounded-md border border-white/5">
                                <div className="flex-1">
                                    ...
                                    />
                                    {mapping.column === 'item' && <p className="text-[10px] text-muted-foreground mt-1 ml-1">Parent Row Container</p>}
                                </div>
                                <div className="flex-1">
                                    <Input
                ...
                <DialogFooter className="mt-6 border-t pt-4">
                    ...
                    <Button onClick={handleSubmit} disabled={isLoading}>
                        {isLoading ? 'Creating...' : 'Save Pipeline'}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>

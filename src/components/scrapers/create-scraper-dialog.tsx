...
interface CreateScraperDialogProps {
    open: boolean
...
export function CreateScraperDialog({ open, onOpenChange, projectId, onSuccess }: CreateScraperDialogProps) {
    const [loading, setLoading] = useState(false)
    const [error, setError] = useState<string | null>(null)

    const [url, setUrl] = useState("")
    const [tableName, setTableName] = useState("")
    const [fields, setFields] = useState<{ name: string, selector: string }[]>([{ name: "title", selector: "" }])
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!data.success) throw new Error(data.error || "Failed to create scraper")
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            required
                        />
                        <p className="text-xs text-muted-foreground">The table will be created automatically.</p>
                    </div>
